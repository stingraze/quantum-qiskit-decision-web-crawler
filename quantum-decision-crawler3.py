...
Update: add --compare-algorithms mode (quantum vs DFS vs BFS sequential comparison with CSV/JSON output).

uv run quantum-decision-crawler3.py \
...
# Comparison mode (3 minutes per algorithm, 9 minutes total):
uv run quantum-decision-crawler3.py \
...
class QuantumCrawler:
    def __init__(
        ...
        dns_timeout: float = 0.0,  # NEW: cap connect-phase budget (DNS is inside connect-phase)
    ):
    ...
    def _respect_host_delay(self, url: str):
        ...
        with self._host_lock:
            ...
            if now < next_ok:
                # We need to wait. Calculate how long, and advance the 'next_ok' 
                # marker so subsequent threads for this host queue up properly.
                sleep_time = next_ok - now
                self._host_next_ok[h] = next_ok + self.per_host_delay
            else:
                # No wait needed. Set the next available time block.
                self._host_next_ok[h] = now + self.per_host_delay

        # Sleep OUTSIDE the lock so other threads can process different hosts
        if sleep_time > 0:
    ...
    def _parse(self, html: str, base_url: str) -> Tuple[str, str, List[Tuple[str, str]]]:
        ...
        # 1MB is more than enough text for crawling purposes.
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(description="Quantum Decision Crawler (hard timeouts + dns-timeout)")
    ...
    p.add_argument("--total-timeout", type=float, default=12.0, help="Hard wall-clock max time (curl --max-time).")
    p.add_argument("--dns-timeout", type=float, default=4.0, help="Caps curl connect-phase time (DNS is inside it).")
